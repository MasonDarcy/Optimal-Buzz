...
fun vanillaClearAndSetNewData(model: SessionViewModel, dao: SessionDao) {
    //clear
    DBUtil.clearAllDb(dao)
    //Insert data
    DBUtil.insertUserData(model, dao)
    DBUtil.insertEntryData(model, dao)
    DBUtil.insertTimeData(model, dao)
    DBUtil.insertDrinkData(model, dao)

}
fun vanillaReinstatePreviousData(model: SessionViewModel, dao: SessionDao) {

    DBUtil.retrieveUser(model, dao)
    DBUtil.retrieveEntries(model, dao)
    DBUtil.retrieveTimeData(model, dao)
    DBUtil.retrieveDrinkData(model, dao)
